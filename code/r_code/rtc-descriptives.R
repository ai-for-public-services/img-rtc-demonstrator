...
##### 
##### # join crashes to lines and then color the lines?
##### #test <- st_join(cam_roads,cam_rtc)
##### cam_roads_sampled <- st_line_sample(st_cast(st_cast(cam_roads, "MULTILINESTRING"),"LINESTRING"), density = 1/50)
##### cam_roads_sampled <- st_cast(cam_roads_sampled, "POINT")
##### cam_split_roads <- lwgeom::st_split(st_cast(st_cast(cam_roads, "MULTILINESTRING"),"LINESTRING"),cam_roads_sampled)
##### 
##### 
##### # can get the nearest crash for each road segment
##### cam_split_roads$nearest_site <- cam_rtc$rtc_count_total[st_nearest_feature(cam_split_roads, cam_rtc)]
##### 
##### test <- st_join(cam_split_roads,cam_rtc,join=st_is_within_distance, dist=2)
##### 
##### tm_shape(test) +
#####   tm_lines("rtc_count_total")


# can get the nearest road for each crash pretty easy but not sure how much that helps me...
# cam_rtc$nearest_road <- cam_roads$osm_id[st_nearest_feature(cam_rtc, cam_roads)]
# 
# cam_rtc %>% 
